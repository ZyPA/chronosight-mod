    ...
    init() {
      ...
      m.setup = function () {
        if (!m.loaded && Game.Objects["Temple"].minigameLoaded) {
          m.start();
          m.updateBest();
          Game.registerHook("check", m.updateBest);
      ...
      m.getCycle = (id) => (id !== -1 ? m.cycles[id] : 0);
      ...
      m.tooltip = function (id) {
        ...
        const trend = Math.abs(slope) < 0.1 ? "flat" : slope < 0 ? "down" : "up";
        const isNegative = effect < 0;
        const isBest = m.state.best == id;
        ...
        <span
          style="
            ...
            ${isBest ? "color:#fc0;" : ""}
          "
          ${isBest ? "" : `class="${isNegative ? "red" : "green"}"`}
        >
          ${effectString}
      ...
      m.cycleData = (cycle) => {
        ...
        return {
          ...
          hourlyAverage: (amplitude / omega) * (Math.cos(phase) - Math.cos(phase + omega)),
        };
      };
      ...
      m.calculateBest = () => {
        const currentCycle = m.getCycle(m.ages.slot);
        const currentData = m.cycleData(currentCycle);
        const current = [currentData.effect, currentData.hourlyAverage, m.ages.slot];
